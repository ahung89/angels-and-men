...
float fractal3D(vec3 x)
{
	...
	float result = perlin3D(x * freq) * ampl;
	freq *= frequencyRatio;
	accum += ampl;
	ampl *= ratio;

	result += perlin3D(x * freq) * ampl;
	freq *= frequencyRatio;
	accum += ampl;
	ampl *= ratio;
...
void main() 
{
    ...
    vec3 pos = position;
    // pos.z += sin(time + pos.x ) * uv.y * clamp(pos.x,0.0,1.0);
    // pos.y -= pos.x * cos(time + pos.x) * uv.y * .25 * clamp(pos.x,0.0,1.0);

    vec4 wsPos = modelMatrix * vec4(pos, 1.0);

    float noise = fractal3D(wsPos.xyz * .25 + vec3(0.0, 0.0, time * .75)) + sin(time * .25 + wsPos.x + wsPos.y + wsPos.z) * .1;

    pos.z += noise * uv.y * clamp(pos.x * .3, 0.0, 1.0);
    pos.y -= noise * uv.y * clamp(pos.x * .3, 0.0, 1.0) * 3.0;
